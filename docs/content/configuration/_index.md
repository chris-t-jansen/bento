...
The config is divided into four sections, each documented in its own page:

- **[`[output]`](/configuration/output)** — container format, destination directory, output filename, conflict resolution, and container metadata.
- **[`[video]`](/configuration/video)** — encoder choice, CRF, preset, and source preprocessing (crop, deinterlace, detelecine, denoise, resize).
- **[`[audio]`](/configuration/audio)** — codec, bitrate, mixdown, loudness normalization, and per-track configuration.
- **[`[subtitles]`](/configuration/subtitles)** — soft mux, burn-in, external sidecars, ASS style filtering, and track subtraction.
