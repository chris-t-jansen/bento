...
| `force_8bit` would clamp a genuinely HDR source to 8-bit SDR | config-implication† | `[video].warn_hdr_force_8bit` |
...
† `warn_unnormalized_downmix` and `warn_hdr_force_8bit` are classed as config-implication — each gets a sticky config field because suppressing it is an intentional, persistent choice about a configuration — but unlike the other config-implication warnings they depend on a runtime fact (the source's channel count, or its color primaries/transfer characteristics respectively), so they can only fire once the source is probed. `warn_unnormalized_downmix` was the first warning of this "config-implication gated by a runtime fact" shape; `warn_hdr_force_8bit` is the second.
...
- `force_8bit` — when `true` (default), forces 8-bit 4:2:0 output (`-pix_fmt yuv420p`) regardless of the source's native pixel format. Many Blu-ray/anime sources are 10-bit (`yuv420p10le`), encoded that way purely for compression efficiency rather than for HDR — left unforced, ffmpeg inherits the source's bit depth and silently produces a High10-profile H.264 stream that Pi-class Jellyfin clients cannot hardware-decode, forcing a software transcode on playback instead of direct-play. This is classed under core encoding rather than source preprocessing (below) because it's inherent to the encode step itself, the same way `crf`/`tune` shape output quality, rather than an optional filter pass. Set to `false` to preserve the source's native pixel format — e.g. for archival encodes where Pi/browser direct-play compatibility isn't a goal.
- `warn_hdr_force_8bit` — when `true` (default), warns when `force_8bit` is about to clamp a genuinely HDR source (BT.2020 color primaries plus PQ/`smpte2084` or HLG/`arib-std-b67` transfer characteristics) to 8-bit SDR. `force_8bit` exists for the common case — a 10-bit-but-SDR anime source, where 8-bit clamping is exactly the right silent fix — but a real HDR source is different: clamping it without tone mapping doesn't just fail to direct-play, it looks wrong (crushed or washed-out color). Bento does not tone-map (see "HDR → SDR tone mapping" under Out of Scope); this warning exists so that gap is loud rather than silent. Depends on the probed source's color metadata, so — like `warn_unnormalized_downmix` — it can only fire once the source is probed, not at config-validation time. Suppressed by `[video].warn_hdr_force_8bit = false` or `--no-warn-hdr-force-8bit`.

  **Known limitation:** detection relies entirely on the source file's own `color_primaries`/`color_transfer` tags, as reported by `ffprobe`. A source that's genuinely HDR but has those tags stripped, missing, or generic (`"unknown"`) will not trigger this warning — Bento has no way to detect HDR from pixel data alone, and deliberately doesn't try to (that's a fundamentally different, much heavier kind of analysis than a transcode tool like Bento is scoped for). This is a false-negative risk to be aware of, not a bug: most real-world rips carry accurate color tags, and a source with stripped metadata is already an edge case outside what tagging-based detection can ever catch. If you work with sources you know or suspect are HDR, don't rely solely on this warning — verify with `ffprobe` yourself before trusting the default `force_8bit = true` behavior on them.
...
| config-implication | `[video].warn_hdr_force_8bit` | `--no-warn-hdr-force-8bit` |
...
- **HDR → SDR tone mapping and color-space handling.** Increasingly relevant as HDR anime ships, but well-understood territory that can be added later without schema disruption. Bento detects and warns about the specific failure mode (`[video].warn_hdr_force_8bit`, see `[video]` above) but does not tone-map — an HDR source today is either left alone (`force_8bit = false`) or crudely clamped to 8-bit SDR (`force_8bit = true`, the default), with no color-correct remapping in between.
